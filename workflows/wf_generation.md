...
### Step 7: 生成后强制自检(GENERATION_CHECKLIST)

> ⛔ **生成完成后、进入 VERIFYING 之前,必须逐条过以下清单。缺一不可,未全部 ✅ 不得进入校验阶段。** 这是固定流程,不是可选建议。
...
□ 1. 合规三层全读了吗?
   → 通用合规 + 险种专项 + 产品专属,三层都读过且无阻断项

□ 2. 脚本原子化了吗?
   → atomic_scripts.json 已产出,台词逐字未改,括号警示语已剥离

□ 3. ⛔ 脚本保真检查(高频翻车点!)
   → 分镜/Prompt 里的台词与 atomic_scripts.json 逐字比对:
   ...
   → 检查方法:把每条 Prompt 的台词段({...}内)拼起来,与 atomic 原文做 diff,
     diff 非空 = ❌ FAIL,列出具体改了哪几个字
   → 铁律:用户给的台词一个字不改、一个字不加、一个字不删。
     精简/润色/重组/同义替换全部禁止。

□ 4. 前贴/成片判定了吗?
   → 按 router §5 判定,前贴无警示语/成片已逐句挂载

□ 5. 分镜走了吗?(出片路径)
   → storyboard.md 已产出且用户已确认(SCRIPT_ONLY 纯脚本出口豁免)

□ 6. 角色参考图确认了吗?(路径二)
   → 四视图+场景图已展示给用户,用户确认外貌/年龄/服装无误
   → (路径一/文生视频跳过此项)

□ 7. READY 确认门过了吗?
   → 分镜+合规报告+预估时长+角色参考图+脚本标注文档(成片),用户确认

□ 8. 视频文件齐全了吗?
   → 每条脚本都有对应 mp4(素材/分段 或 成片/拼接后)
   → extend 模式的多段已拼接为完整成片
   → 无 FINAL_FAILED 未处理项
...
**处理规则**:
- **全部 ✅** → 展示"自检通过",进入 VERIFYING
- **有 ❌** → 列出未通过项,**不进 VERIFYING**,先修:
  - 1-6 项(流程缺步)→ 回退到对应阶段补做
  - 7-9 项(产物问题)→ 重新生成/补写日志/调整目录
  - 10 项(时长异常)→ 标记疑似,进 VERIFYING 重点检查
