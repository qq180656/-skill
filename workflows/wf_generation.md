...
### Step 1: 时长预算
- 公式: **修正后总时长 = 口播总字数(去括号后) ÷ 基准语速 + 停顿(逗号/句末/角色切换) + 纯动作镜头总时长 + 段末缓冲(1.5-2s)**（算法见 `segmentation_packaging.md` Step 6）
...
- 遵循四层分层结构（完整规范见 `prompt_craft_guide.md`）:
  1. 素材与音色绑定层 — 参考图逐镜传(按 character_lock_protocol.md)；有音色锚定的角色同时通过 AudioList 传入 voice mp3
  2. 全局概述层（一句话）
  3. 画面阶段描述层（含时间锚点/台词/动作/音效）
  4. 约束层见 prompt_craft_guide.md 第四层(唯一权威)
...
□ 7. ⛔ 多版本矩阵差异确认（N≥3条/脚本时，2026-09-21新增）
     → 必须先出**1条全差异样片**（不同人设组合+不同场景+不同分镜+不同开场语法）
       给用户看过并确认"这就是N条之间的差异粒度"后，才允许放量跑全部N条
     → 严禁按字面"变体/版本"理解就开工（260920批实测：理解为"只换光影的V1-V4变体"
       跑了几条即被叫停，用户实际要"人物场景都不一样"，80条全部返工）
     → 差异维度清单逐项确认：人设组合/场景/分镜编排/开场语法/光影色调——哪些必须不同、哪些可以复用
```
