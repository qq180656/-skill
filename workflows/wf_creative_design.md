...
> **💡 可选能力提醒（进入创意设计时告知用户,用户说用才加载）**：
> - "需要**叙事模板**参考吗?" → 加载 `narrative_archetypes.md`（四大险种叙事原型+七拍/十拍节拍表）
> - "需要**电影化叙事工具**吗?" → 加载 `cinematic_narrative_tools.md`（命题先行/因果连续/蒙太奇/色调对比）
> - "需要**系统化色彩方案**吗?" → 加载 `visual_style_system.md`（60:30:10配色+四大险种12色+风格签名串）
> - "需要看**优质案例**怎么拍的吗?" → 加载 `reference_video_analysis.md`（19条保险视频逐帧拆解）
- 识别意图层: 题材 目的 面向人群
...
- **叙事结构时长匹配**：
  - 15-30s 前贴 → 钩子型：3s 引入 + 8s 铺垫 + 4s 产品名 + 2s 收尾
...
- **开场方式选型指引**（参考 `opening_hooks_library.md` 完整库，含每种方式的画面描述+写法要点+Prompt示例+避坑提示）：
  ...
  - 按时长推荐：
    - 前贴15-30s → 直面镜头Hook / 面部大特写直入 / 手持道具口播 / 手机提示音触发 / 问答互动
...
- **角色数量上限**：
  - 15-30s 前贴 ≤ 2 角色；30s 成片 ≤ 3 角色；60s 成片 ≤ 4 角色
