...
def generate_character_sheet(
    ...
    prompt = (
        ...
        f"同一角色的人物正面头肩特写、正面、侧面、背面三个完整全身视图横向排列，"
        f"人物特写只显示头部至肩部，不显示腰部、腿部和脚，\n"
        f"三个全身视图从头顶完整展示到脚尖，自然站立、双足平踏地面、平视镜头，\n"
        f"四视图角色特征与服装细节完全一致，色彩准确统一。\n"
        f"纯白色背景，均匀平光照明，无明显阴影。\n"
        f"{style}。\n"
        f"正面、侧面、背面三个全身视图严禁裁切下半身，人物特写禁止生成成全身视图，"
        f"禁止生成任何背景、文本、标注、字幕。\n"
        f"比例:16:9"
    )
