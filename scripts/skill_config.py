...
> **平台适配声明**：Key/网关相关配置（GW_KEYS/LLM_KEYS/BASE_DOUBAO/LLM_URL）
> 为外部 BlueAI 网关环境设计，在小云雀平台中不适用（平台用积分管理替代Key池）。
> 非API依赖的配置（语速/水印/发音规避/CSV字段/产品ID/前贴参数）可直接复用。
> 平台工具映射见 `core/platform_adapter.md`。
...
# 语速（字/秒）——按产品区分
# 2026-09-21 Seedance 语速规则 v2（ASR实测校准，用户确认）：
#   基准 7→6（中青年/默认）；老人/儿童 5。Seedance 按时长预算自动调速，
#   段时长必须 ≥ 台词字数÷语速 + 停顿 + 1.5s 收尾，否则模型加速赶话（实测10.7字/s）。
SPEED = {
    "好医保·中老年长期医疗": 6,
    "好医保·中老年长期医疗(2026版)": 6,
    "好医保·中老年长期医疗(体验版)": 6,
