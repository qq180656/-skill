...
| **批量链路状态** | JSON | {project}/_batch_state.json | **跨会话长链路（提示词→图→双审→批量→叠化→终验）的统一断点状态（2026-09-21新增）。单写入口：每完成一步立即更新。字段：{提示词:{状态,文档路径}, 图池:{状态,目录,张数}, 双审:{保真,LLM审}, 批量:{进度,台账路径,费用}, 后处理:{xfade,终验}}。任何会话恢复（含分类器故障/额度打满/进程中断后）必须先读它再决定下一步，禁止靠对话记忆盘点（260920批实测：链路跨多次中断，每次恢复重新盘点耗时且易漏）** |
